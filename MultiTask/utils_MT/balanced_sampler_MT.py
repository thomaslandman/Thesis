...
class BalancedSampler(UniformSampler):
    ...
    def __init__(self, reader, window_sizes, batch_size=1, windows_per_image=1, queue_length=10, name='balanced_sampler'):
        UniformSampler.__init__(self, reader=reader, window_sizes=window_sizes, batch_size=batch_size,
                                windows_per_image=windows_per_image, queue_length=queue_length, name=name)
        tf.logging.info('Initialised balanced sampler window instance')
        self.window_centers_sampler = balanced_spatial_coordinates


def balanced_spatial_coordinates(n_samples, img_spatial_size, win_spatial_size, sampler_map):
    """
    ...
    for label, count in zip(unique_labels, label_counts):
        ...
        try:
            ...
            tf.logging.fatal("unable to choose sampling window based on the current frequency map.")
            raise
