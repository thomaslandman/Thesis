...
def GenerateBoxplots(xlsxLoader, out_file):
    ...
    ax.set_xlabel('DSC')
    doPlot(xlsxLoader, settings, ax, xlsxLoader.exp_gtv['DSC'], np.arange(0, 1.1, 0.1), 0, 1, boxplotLoc='lower left')
    ...
    ax = axes[0,2]
    ax.set_xlabel('95%HD (mm)')
    doPlot(xlsxLoader, settings, ax, xlsxLoader.exp_gtv['HD'], np.arange(0, 21, 2), 0, 20)

    ax = axes[1,0]
    ax.set_ylabel('Seminal Vesicles')
    doPlot(xlsxLoader, settings, ax, xlsxLoader.exp_sv['DSC'], np.arange(0, 1.1, 0.1), 0, 1, boxplotLoc='lower left')

    ax = axes[1,1]
    doPlot(xlsxLoader, settings, ax, xlsxLoader.exp_sv['MSD'], np.arange(0, 11, 1), 0, 10)

    ax = axes[1,2]
    doPlot(xlsxLoader, settings, ax, xlsxLoader.exp_sv['HD'], np.arange(0, 31, 2), 0, 20)

    ax = axes[2,0]
    ax.set_ylabel('Rectum')
    doPlot(xlsxLoader, settings, ax, xlsxLoader.exp_rectum['DSC'], np.arange(0, 1.1, 0.1), 0, 1, boxplotLoc='lower left')

    ax = axes[2,1]
    doPlot(xlsxLoader, settings, ax, xlsxLoader.exp_rectum['MSD'], np.arange(0, 11, 1), 0, 10)

    ax = axes[2,2]
    doPlot(xlsxLoader, settings, ax, xlsxLoader.exp_rectum['HD'], np.arange(0, 61, 2), 0, 20)


    ax = axes[3,0]
    ax.set_ylabel('Bladder')
    doPlot(xlsxLoader, settings, ax, xlsxLoader.exp_bladder['DSC'], np.arange(0, 1.1, 0.1), 0, 1, boxplotLoc='lower left')

    ax = axes[3,1]
    doPlot(xlsxLoader, settings, ax, xlsxLoader.exp_bladder['MSD'], np.arange(0, 11, 1), 0, 10)

    ax = axes[3,2]
    doPlot(xlsxLoader, settings, ax, xlsxLoader.exp_bladder['HD'], np.arange(0, 41, 2), 0, 20)
    ...
    fig.savefig(os.path.join(out_file, 'boxplot.png'), dpi=1000)
    fig.savefig(os.path.join(out_file, 'boxplot.pdf'), dpi=1000)
    print("Generated Boxplot ", os.path.join(out_file, 'boxplot.pdf'))
